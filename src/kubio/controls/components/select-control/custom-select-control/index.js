 ...
 */
import { usePopupNestingContext } from '@kubio/core';
							...
							<Button
								{...getToggleButtonProps({
									...
									className: classnames([
										...
										{
											'h-select-control__button--placeholder':
												noItemSelected,
										},
									]),
									isSmall: true,
...
function stateReducer({ selectedItem }, { type, changes, props: { items } }) {
	switch (type) {
...
function getPopoverSlotNode() {
	...
	if (!slot) {
		...
		slot.setAttribute(
			'style',
			'position:absolute;top:0;left:0;z-index:10000000'
		);
