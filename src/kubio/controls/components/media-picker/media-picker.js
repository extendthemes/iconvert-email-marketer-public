...
const MediaPicker = ( {
	...
	buttonLabel = __( 'Change image', 'iconvert-email-marketer' ),
	removeButtonLabel = __( 'Remove image', 'iconvert-email-marketer' ),
	label,
	withReset = false,
	toggable = false,
	toggleValue = false,
		...
		<>
			...
			>
				...
				{ ( ! toggable || ( toggable && toggleValue ) ) && (
					<>
						<MediaUpload
							title={ __( 'Select image', 'iconvert-email-marketer' ) }
							onSelect={ onChange }
							allowedTypes={ [ type ] }
							value={ mediaId }
							render={ ( { open } ) => (
