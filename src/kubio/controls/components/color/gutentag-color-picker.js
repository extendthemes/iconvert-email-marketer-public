		...
		<Flex align={'center'} gap={0}>
			...
			<FlexItem>
				<InputNumber
					...
					onChange={(nextValue) => {
						onChangeComplete({
							rgb: tinycolor(color).setAlpha(nextValue).toRgb(),
						});
					}}
				/>
			</FlexItem>
...
const GutentagColorPicker = ({
	...
	useEffect(() => {
		const nextValue = normalizeNextColor(value, alpha);
		...
		<>
			...
			<BaseControl label={label} className={containerClassName}>
				<ChromePicker
					...
					disableAlpha={true}
					onChange={(nextValue) => onColorChange(nextValue, true)}
